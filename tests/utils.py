...
def assert_valid_meta_for_create_request(meta, created_by: str = "unknown", created_via: str = "api"):
    ...
    assert "updated_via" in meta
    assert meta["updated_via"] is None

def assert_valid_meta_for_update_request(meta, updated_by: str = "unknown", updated_via: str = "api"):
    assert meta is not None
    ...
    assert meta.get("updated_via") == updated_via
...
def mock_context(via: str = "api"):
    """
    Returns a mock context.
    """
    context = { "user": "test-user", "via": via }
    return context

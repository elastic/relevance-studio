...
- **`task.strategies.docs`** - The `_source` of [strategy](#strategies) documents to include in [evaluations](#evaluations). Primarily intended to be used by the strategy testing components of the [UI](docs/reference/architecture?id=application).
...
*Note: Evaluations don't have an Update API. [Workers](docs/reference/architecture?id=application) handle the updates automatically.*